...
with torch.no_grad():
    ...
    if opts.trainer == 'MUNIT':
        ...
        # 입력 파일명에서 이름만 추출 후 UNIT_ 접두어 붙이기
        input_basename = os.path.basename(opts.input)  
        output_filename = f'UNIT_{input_basename}'     

        output_path = os.path.join(opts.output_folder, output_filename)

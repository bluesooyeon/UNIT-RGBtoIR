...
def get_data_loader_list(root, file_list, batch_size, train, new_size=None,
                         ...
                         grayscale=False, gray_channels=3):
    # NOTE: Grayscale은 ToTensor 이전에 적용되어야 함
    transform_list = []
    if grayscale:
        transform_list.append(transforms.Grayscale(num_output_channels=gray_channels))
...
def load_vgg16(model_dir=None):
    """
    최신 torchvision의 VGG16 모델을 불러옵니다.
    기존 `.t7` 모델 로드 및 변환 과정은 제거됩니다.
    """
    # torchvision에서 사전 학습된 VGG16 로드
    vgg = models.vgg16(pretrained=True).features[:23]  # conv4_3까지만 사용 (Perceptual loss 등에서 일반적)
    vgg.eval()  # 학습되지 않도록 설정
